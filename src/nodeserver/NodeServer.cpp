...
#include "NodeSession/NodeSession.hpp"
#include "Logger.hpp"

using boost::asio::ip::tcp;

NodeServer::NodeServer(boost::asio::io_context &io_context,
            ...
            [self = shared_from_this()](boost::system::error_code ec, tcp::socket socket) {
                ...
                if (ec) {
                    if (ec != boost::asio::error::operation_aborted &&
                        ...
                        log->error("[NodeServer] Accept failed: {}", ec.message());
                    }
                    return;
                }

                auto session = std::make_shared<NodeSession>(std::move(socket), self);

                {
                    std::lock_guard<std::mutex> lock(self->sessions_mutex_);
                    self->sessions_.insert(session);
                    log->info("[NodeServer] New client connected. (RelayServer)");
...
void NodeServer::stop() {
    ...
    if (ec && ec != boost::asio::error::operation_aborted && ec != boost::asio::error::eof) {
        log->error("[NodeServer] Failed to cancel acceptor: {}", ec.message());
    }
    ...
    {
        std::unordered_set<std::shared_ptr<NodeSession>> sessions_copy;
        {
...
void NodeServer::remove_session(std::shared_ptr<NodeSession> session) {
    std::lock_guard<std::mutex> lock(sessions_mutex_);
    sessions_.erase(session);
    log_session_count();
}

void NodeServer::log_session_count() {
    Logger::get()->info("[NodeServer] Active sessions: {}", sessions_.size());
}
