...
struct DbcTotemCategory {
    ...
    uint32_t totemcategorytype;
    int32_t totemcategorymask;
};

template<>
struct PgRowMapper<DbcTotemCategory> {
    static DbcTotemCategory map(const pqxx::row& r) {
        DbcTotemCategory row;
        ...
        row.totemcategorytype = r["totemcategorytype"].as<uint32_t>();
        row.totemcategorymask = r["totemcategorymask"].as<int32_t>();
        return row;
    }
};