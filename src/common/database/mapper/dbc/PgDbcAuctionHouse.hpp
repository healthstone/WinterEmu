...
/** dbc_auctionhouse **/
struct DbcAuctionHouse {
    uint32_t ID;
    uint32_t FactionID;
    uint32_t DepositRate;
    uint32_t ConsignmentRate;
...
template<>
struct PgRowMapper<DbcAuctionHouse> {

    static DbcAuctionHouse map(const pqxx::row& r) {
        DbcAuctionHouse row;

        row.ID = r["id"].as<uint32_t>();
        row.FactionID = r["faction_id"].as<uint32_t>();
        row.DepositRate = r["deposit_rate"].as<uint32_t>();
        row.ConsignmentRate = r["consignment_rate"].as<uint32_t>();
