...
typedef std::unordered_map<uint32_t /*ID*/, SpellCategoryDBC> SpellCategoryDBCMap;
typedef std::unordered_map<uint32_t /*ID*/, SpellDifficultyDBC> SpellDifficultyDBCMap;
...
typedef std::unordered_map<uint32_t /*ID*/, WorldSafeLocsDBC> WorldSafeLocsDBCMap;
...
public:
    ...
    SpellCategoryDBC const* getSpellCategoryDBC(uint32_t ID)
    {
        auto itr = _spellCategoryMap.find(ID);
    ...
    SpellDifficultyDBC const* getSpellDifficultyDBC(uint32_t ID)
    {
        auto itr = _spellDiffucultyMap.find(ID);
        if (itr != _spellDiffucultyMap.end())
            return &itr->second;
        return nullptr;
    }

    SpellDurationDBC const* getSpellDurationDBC(uint32_t ID)
    {
        auto itr = _spellDurationMap.find(ID);
        if (itr != _spellDurationMap.end())
            return &itr->second;
        return nullptr;
    }

    SpellFocusObjectDBC const* getSpellFocusObjectDBC(uint32_t ID)
    {
        auto itr = _spellFocusObjectMap.find(ID);
        if (itr != _spellFocusObjectMap.end())
            return &itr->second;
        return nullptr;
    }

    uint32_t getSpellItemEnchantmentDBCHighestID() const { return _spellItemEnchantmentHighestID; }
    SpellItemEnchantmentDBC const* getSpellItemEnchantmentDBC(uint32_t ID)
    {
        auto itr = _spellItemEnchantmentMap.find(ID);
    ...
    TransportAnimationDBC const* getTransportAnimationDBC(uint32_t ID)
    {
        auto itr = _transportAnimationMap.find(ID);
    ...
    TransportRotationDBC const* getTransportRotationDBC(uint32_t ID)
    {
        auto itr = _transportRotationMap.find(ID);
    ...
    WorldSafeLocsDBC const* getWorldSafeLocsDBC(uint32_t ID)
    {
        auto itr = _worldSafeLocsMap.find(ID);
...
private:
    ...
    void load_SpellCategory();              // load SpellCategory.dbc
    void load_SpellDifficulty();            // load SpellDifficulty.dbc
