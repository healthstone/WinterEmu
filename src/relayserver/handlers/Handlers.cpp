...
void Handlers::dispatch(std::shared_ptr<GameSession> session, std::shared_ptr<WoWPacket> p) {
    ...
    switch (opcode) {
        ...
        case WoWOpcodes::CMSG_UPDATE_ACCOUNT_DATA: {
            HandleUpdateAccountData(session, p);
            break;
        }
...
void Handlers::HandleUpdateAccountData(const std::shared_ptr<GameSession>& session, std::shared_ptr<WoWPacket> p) {
    auto log = Logger::get();
    ...
    try {
        ...
        if (decompressedSize == 0) {
            ...
            session->setAccountData(static_cast<AccountDataType>(type), timestamp, "");
        ...
        session->setAccountData(static_cast<AccountDataType>(type), timestamp, data);
